# Documentation Index

🎯 **You are here**: Docs Directory | [← README](../README.md) | [Navigation Guide →](DOCUMENTATION_NAVIGATION.md)

---

## Quick Links

- **Start Here**: [DOCUMENTATION_NAVIGATION.md](DOCUMENTATION_NAVIGATION.md) - Find your reading path
- **Requirements**: [../project/REQUIREMENTS_MATRIX.md](../project/REQUIREMENTS_MATRIX.md) - V&V status
- **Getting Started**: [../README.md](../README.md) - Project overview

---

## Technical Documentation

### Authentication & Security
- [AUTHENTICATION_REFACTORING.md](AUTHENTICATION_REFACTORING.md) - Auth system design (3 modes)
- [DEPLOYMENT_MODES.md](DEPLOYMENT_MODES.md) - Local, Ldap, LdapSSO modes
- [KEYCLOAK_SETUP.md](KEYCLOAK_SETUP.md) - SSO configuration guide
- [KEYCLOAK_QUICKREF.md](KEYCLOAK_QUICKREF.md) - Quick SSO reference
- [KEYCLOAK_INTEGRATION.md](KEYCLOAK_INTEGRATION.md) - SSO integration details

### Deployment & Operations
- [PRODUCTION_CONFIG.md](PRODUCTION_CONFIG.md) - Production configuration
- [DOCKER_FIRST_REFACTORING.md](DOCKER_FIRST_REFACTORING.md) - Platform strategy
- [../scripts/README.md](../scripts/README.md) - Automation scripts

### Navigation
- [DOCUMENTATION_NAVIGATION.md](DOCUMENTATION_NAVIGATION.md) - Navigation guide with reading paths

---

## Project Management Docs

Located in `../project/`:
- [REQUIREMENTS.md](../project/REQUIREMENTS.md) - All requirements (43 total)
- [REQUIREMENTS_MATRIX.md](../project/REQUIREMENTS_MATRIX.md) - V&V matrix ⭐ START HERE
- [TRACEABILITY.md](../project/TRACEABILITY.md) - Implementation mapping
- [VERIFICATION_VALIDATION.md](../project/VERIFICATION_VALIDATION.md) - V&V strategy
- [DEVIATIONS.md](../project/DEVIATIONS.md) - Approved deviations

---

## Reports

Auto-generated in `../reports/`:
- TEST_TRACEABILITY.md - Test coverage (run `dotnet test` to generate)
- LICENSE_COMPLIANCE.md - Dependency licenses

---

**Navigation**: [← README](../README.md) | [Navigation Guide →](DOCUMENTATION_NAVIGATION.md) | [Requirements →](../project/REQUIREMENTS_MATRIX.md)
